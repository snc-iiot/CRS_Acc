...
const HomePage: FC = () => {
  ...
  const DataObjectivePurchasingRatio = [
    {
      ...
      value: 0,
      ซื้อมาขายไป:
        dataObjectivePurchasingRatio?.filter(({ objective_purchasing }) => objective_purchasing === "trade")?.[0]
          ?.amount ?? 10,
      ผลิตงาน:
        dataObjectivePurchasingRatio?.filter(({ objective_purchasing }) => objective_purchasing === "produce")?.[0]
          ?.amount ?? 10,
      อื่นๆ:
        dataObjectivePurchasingRatio?.filter(({ objective_purchasing }) => objective_purchasing === "other")?.[0]
          ?.amount ?? 10,
  ...
  const DataMainCustomers = [
    {
      id: "ไทย",
      label: "ไทย",
      value: dataMainCustomerRatio?.filter(({ main_customer }) => main_customer === "internal")?.[0]?.amount ?? 0,
      color: "#1F2C57",
    },
    {
      id: "ต่างประเทศ",
      label: "ต่างประเทศ",
      value: dataMainCustomerRatio?.filter(({ main_customer }) => main_customer === "foreign")?.[0]?.amount ?? 0,
      color: "#F2C94C",
    },
  ];
  ...
  const TotolRegisWest = dataRegisCount
    ?.map(({ regis_count, province }) => (province !== "RAYONG" ? 0 : regis_count ?? 0))
    ?.reduce((accumulator, currentValue) => {
      return accumulator + currentValue;
    }, 0);
  const TotolRegisEast = dataRegisCount
    ?.map(({ regis_count, province }) => (province !== "SAMUTPAKAN" ? 0 : regis_count ?? 0))
    ?.reduce((accumulator, currentValue) => {
      return accumulator + currentValue;
    }, 0);

  const dataRegisWest = dataRegisCount
    ?.filter(({ province, regis_count }) => province !== "RAYONG" && regis_count > 0)
      ...
      <section className="grid h-full grid-cols-8 grid-rows-2 gap-2">
        <article className="col-span-6 row-span-1 flex h-full flex-col">
          {dataRegisStat?.length === 0 ? (
            <article className="flex h-full w-full flex-col items-center justify-center rounded-md border">
              <text className="text-sm">
                {isFetchingRegisCount ? "กำลังโหลดข้อมูล..." : "ยังไม่มีข้อมูลการลงทะเบียน"}
              </text>
            </article>
          ) : (
            <section className="grid h-full w-full grid-cols-8 grid-rows-2 gap-2 ">
              <article className="col-span-2 row-span-2 flex h-full flex-col items-center justify-center rounded-md border p-2">
                <h3 className="text-md font-bold">จำนวนลูกค้า SNC รวมทั้งเครือ</h3>
                <h1
                  className={`${
                    TotolRegis < 1000
                      ? "text-[5rem]"
              ...
              <article className="relative z-10 col-span-2 flex h-full flex-col items-center  justify-center rounded-md border p-2">
                <h3 className="text-md absolute left-0 top-[-10px] bg-white px-2 font-bold">East :</h3>
                <h1
                  // className={`${
                  //   TotolRegis < 1000
                  //     ? "text-[6rem]"
              ...
              <article className="relative col-span-2 flex h-full flex-col items-center justify-center rounded-md border p-2">
                <h3 className="text-md absolute left-0 top-[-10px] bg-white px-2 font-bold">West :</h3>
                <h1
                  // className={`${
                  //   TotolRegis < 1000
                  //     ? "text-[6rem]"
          ...
          <section className="relative flex h-full w-full flex-col">
            {dataMainCustomerRatio?.length === 0 ? (
              <article className="flex h-full w-full flex-col items-center justify-center">
                <text className="text-sm">
                  {isFetchingShareHolderRatio ? "กำลังโหลดข้อมูล..." : "ยังไม่มีข้อมูลการลงทะเบียน"}
                </text>
              </article>
            ) : (
              <>
                <article className="flex h-[99%] w-full flex-col  items-center justify-center">
                  <PieChartComponents
                    data={Datanationality?.reduce((acc: any, cur) => {
                      const index = acc.findIndex((item: { id: string }) => item.id === cur.id);
                      if (index === -1) {
                        acc.push({
                          id: cur.id || "",
                          label: cur.id || "",
                ...
                <article className="absolute bottom-0 right-0">
                  {Datanationality?.reduce((acc: any, cur) => {
                    ...
                    ?.slice(0, 5)
                    ?.map(({ id, color }: { id: string; color: string }, i: number) => (
                      <article key={i} className="flex flex-row items-center justify-start gap-2">
                        <div
                  ...
                  {Datanationality?.length > 5 && (
                    <h6 className="mt-[-6px] w-full text-center text-xs font-bold">...</h6>
                  )}
                </article>
              </>
            )}
        ...
        <article className="col-span-2 row-span-1 flex flex-col gap-4 rounded-md border p-2">
          <h3 className="text-md font-bold">ประเภทลูกค้าแบ่งตามวัตถุประสงค์การซื้อสินค้า</h3>
          {dataObjectivePurchasingRatio?.length === 0 ? (
            <article className="flex h-[92%] w-full flex-col items-center justify-center">
              <text className="text-sm">
                {isFetchingObjectivePurchasingRatio ? "กำลังโหลดข้อมูล..." : "ยังไม่มีข้อมูลการลงทะเบียน"}
              </text>
            </article>
          ) : (
        ...
        <article className="relative col-span-4 row-span-1 rounded-md border">
          <section className="flex h-full w-full flex-col gap-2 p-2">
            <article>
              <h3 className="text-md font-bold">ประวัติขึ้นทะเบียนผู้ซื้อแบ่งตามบริษัท </h3>
            </article>
            {data?.length === 0 ? (
              <article className="flex h-full w-full flex-col items-center justify-center">
                <text className="text-sm">
        ...
        <article className="col-span-2 row-span-1 flex h-full flex-col rounded-md border p-2">
          <h3 className="text-md font-bold">ลูกค้าหลักของบริษัทที่ขึ้นทะเบียน</h3>
          {dataMainCustomerRatio?.length === 0 ? (
            <article className="flex h-full w-full flex-col items-center justify-center">
              <text className="text-sm">
                {isFetchingMainCustomerRatio ? "กำลังโหลดข้อมูล..." : "ยังไม่มีข้อมูลการลงทะเบียน"}
              </text>
            </article>
          ) : (
