...
const R3Form: FC = () => {
  ...
  const { commentR3, financialRatio, registration } = useAtomStore();
  ...
  return (
    <div className="relative flex w-full flex-col gap-2">
      <div>
        <h3 className="text-base font-bold">ผลประเมินอัตราส่วนทางการเงิน / Financial Ratio Assessment Report</h3>
      </div>
          ...
          <Table className="w-full border-collapse border-spacing-0 border border-black">
            ...
            {data?.map((table, i) => (
              <TableBody key={i}>
                <TableRow className="text-xs">
                  <TableCell
                    colSpan={financialRatio?.financial_ratios?.[0]?.info?.length + 1}
                      ...
                      {info?.info?.map((data, i) => (
                        <TableCell
                          ...
                          className={cn(
                            ...
                            data?.ratio < 0 ? "font-semibold text-red-500" : "text-black",
                            i === info?.info?.length - 1 ? "border-r-black" : ""
                          )}
                          colSpan={1}
                        >
                          {data?.ratio < 0 ? `(${Math.abs(data?.ratio)})` : data?.ratio}
                        </TableCell>
                      ))}
                    </TableRow>
                  ))}
        ...
        <div className="flex h-96 max-h-96 w-full flex-col gap-2">
              ...
              {commentR3?.comments
                ...
                ?.map((info, i) => (
                  <div className="grid grid-cols-10" key={i}>
                    <p className="col-span-6 text-xs text-black">{info?.comments ?? "-"}</p>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button
                disabled={
                  (role !== "admin" && role !== "approver" && role !== "user") || (registration?.status_no ?? 0) !== 4
                    ? true
                    : false
                }
              >
                เพิ่มข้อเสนอแนะทางการเงิน
              </Button>
            </PopoverTrigger>
            <PopoverContent align="start" side="top" className="h-60 w-72 p-1">
              <main className="flex h-full w-full flex-col gap-2 overflow-hidden p-1">
                <h2 className="px-2 py-1 text-sm font-semibold underline">เพิ่มข้อเสนอแนะ / Add Comments</h2>
                <form
