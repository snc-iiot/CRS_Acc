...
export const renderShortDateWithYearFormat = (
  date: string | Date,
  placeholder?: string,
) => {
...
export const renderShortMonthDate = (
  date: string | Date,
  placeholder?: string,
) => {
  if (!date || date === "") return null;
...
export const getTimestampAfterCurrentTime = (): Array<{
  ...
  for (let i = 0; i < 24; i++) {
    ...
    time.push({
      label: newTime.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      }),
      value: newTime,
...
export const getDatesAfterCurrentDate = (): Array<{
  ...
  for (let i = 0; i < 7; i++) {
    const newDate = new Date(
      Math.round(current.getTime() / (30 * 60 * 1000)) * 30 * 60 * 1000,
    );
    date.push({
